...
async function applyBackgroundEffect(video, canvas, segmentation, bgImages) {
  ...
  if (backgroundType === "blur") {
    ...
    offscreenCtx.drawImage(
      bgImages[backgroundType],
      0,
      0,
      canvas.width,
      canvas.height
    );
    backgroundFrame = offscreenCtx.getImageData(
      0,
      0,
      canvas.width,
      canvas.height
...
function isFingerExtended(landmarks, fingerTip, fingerPip, fingerMcp) {
  const tipToPip = calculateDistance(
    landmarks[fingerTip],
    landmarks[fingerPip]
  );
  const pipToMcp = calculateDistance(
    landmarks[fingerPip],
    landmarks[fingerMcp]
  );
  const tipToMcp = calculateDistance(
    landmarks[fingerTip],
    landmarks[fingerMcp]
  );
...
async function setupTranscription() {
  ...
  if (!("webkitSpeechRecognition" in window)) {
    transcriptionDiv.innerHTML =
      "<strong>Live Transcription:</strong><br>Speech recognition not supported in this browser";
    return;
  }
  ...
  if (!micPermissionRequested) {
    ...
    try {
      ...
      transcriptionDiv.innerHTML =
        "<strong>Live Transcription:</strong><br>Microphone permission denied.";
      return;
    }
  }
  ...
  recognition.onerror = (event) => {
    ...
    transcriptionDiv.innerHTML =
      "<strong>Live Transcription:</strong><br>Error: " + event.error;
  };
...
function setupEventListeners() {
  ...
  document.getElementById("gesture-toggle").addEventListener("click", () => {
    ...
    btn.textContent = isGestureEnabled
      ? "👋 Turn Off Gestures"
      : "👋 Turn On Gestures";
...
// Commented out to prevent automatic camera/mic permission requests
// The AI features will be initialized manually after user enters their name
