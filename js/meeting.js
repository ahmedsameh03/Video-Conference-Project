...
async function startCamera() {
  ...
  try {
    ...
    console.warn("Attempt 1 failed:", error.name, error.message);
    try {
      localStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
      console.log("Attempt 2: Camera only accessed successfully.");
    } catch (error2) {
      console.warn("Attempt 2 failed:", error2.name, error2.message);
      try {
...
ws.onmessage = async (message) => {
  try {
    ...
    console.log(`Received message type: ${data.type}, from: ${data.user}, room: ${data.room}`);
    if (!data.type) return;
    ...
    switch (data.type) {
      ...
      case "offer":
        ...
        if (offerCollision && !isPolite) {
          console.warn(`Offer collision from ${data.user}, dropping offer`);
          return;
        }
        try {
      ...
      case "answer":
        ...
        if (peers[data.user]) {
          ...
          console.warn(`No peer connection found for ${data.user}`);
        }
        break;
        ...
        if (peerConn) {
          if (peerConn.remoteDescription?.type) {
            try {
              ...
              console.log(`Added ICE candidate for ${data.user}`);
            } catch (e) {
...
async function createPeer(user) {
  ...
  peer.oniceconnectionstatechange = () => {
    ...
    if (["failed", "disconnected", "closed"].includes(peer.iceConnectionState)) {
      ...
      if (peer.iceConnectionState === "failed") {
        peer.restartIce();
        console.log(`Restarting ICE for ${user}`);
      }
    }
  };
  ...
  peer.ontrack = (event) => {
    console.log(`Track event for ${user}:`, event);
    console.log(`Received streams:`, event.streams.map(s => ({ id: s.id, active: s.active })));
    if (event.streams && event.streams[0]) {
      addVideoStream(event.streams[0], user);
    } else {
      console.warn(`No streams received from ${user}. Check if tracks are sent.`);
    }
  };
  ...
  if (localStream && localStream.getTracks().length) {
    localStream.getTracks().forEach(track => {
      ...
      if (track.enabled) {
        ...
        console.warn(`Track ${track.kind} is disabled for ${user}. Enabling it...`);
        track.enabled = true;
        const sender = peer.addTrack(track, localStream);
    ...
    console.error("No localStream available or no tracks for peer:", user);
    await startCamera();
    localStream.getTracks().forEach(track => {
      const sender = peer.addTrack(track, localStream);
      console.log(`Retried and added ${track.kind} track for ${user}`);
    });
  }
    ...
    Object.entries(peers).forEach(([user, peer]) => {
      if (peer instanceof RTCPeerConnection) {
        ...
        if (sender) {
          ...
          console.warn(`No video sender found for ${user}`);
        }
      } else {
        console.warn(`Invalid peer object for ${user}:`, peer);
      }
    });
